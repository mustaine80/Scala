object FileMatcher {
	private def filesHere = (new java.io.File("bin")).listFiles
...
object HigherOrderedFileMatcher {
	...
	private def filesMatching2(matcher: String => Boolean) = {
		  filesHere.filter((f: java.io.File) => matcher(f.getName))
		}
...
object MainFunctionalEx2 extends App {
	// import FileMatcher._
	import HigherOrderedFileMatcher._
	println("현재 디렉토리에서 확장자가 class 인 파일 목록입니다. " + filesEnding(".class").toList)
