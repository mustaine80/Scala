// abstract class Tree
case class Branch(elem:Int, left: Tree, right: Tree) extends Tree
...
abstract class Tree extends Traversable[Int] {
	def foreach[U](f: Int => U) = this match {
		...
		case Branch(e, l, r) => l foreach f; f(e); r foreach f 
	}
}
...
object collectionEx3 extends App {
	...
	///	TODO: reduceLeft 가 좀 더 generic 한 foldLeft 라고 하는데, 사용처를 잘 모르겠음
	...
	val tree = Branch(4, Branch(2, Node(1), Node(3)), Branch(5, Node(6), Node(7)))
	//tree foreach println
	println(tree.toIterable.iterator.hasNext)
	tree.map(2*_).foreach(println)
}