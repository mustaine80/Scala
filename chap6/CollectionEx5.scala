...
object CollectionEx extends App {
  ...
  val b = List(1,2,3,4,6)
  println(s"(a corresponds b)(_ == _) : ${(a corresponds b)(_ == _)}")

  //	중복 집합 연산
  println(s"a intersect b : ${a intersect b} ")
  println(s"a diff b : ${a diff b}")
  println(s"a union b : ${a union b}")  /// a ++ b
  println(s"xs.distinct : ${xs.distinct}")
}